...
**abox has not had third-party security review.** Version 0.1.4. One author. The
...
| Substitute an official catalog server via an imported catalog file | every file under `~/.docker/mcp/catalogs/` is merged in filename order and the last wins; abox records which files defined each name | **doctor fails** `servers.catalog-shadowing` for any *declared* server defined more than once | `tests/test_catalog_and_cli.py::test_doctor_fails_when_a_declared_server_is_shadowed` |
| Plant a git hook, or any `.git/config` key that executes | `.git/hooks` masked by default; every `.git/config` key that is not on a benign-by-construction list is fingerprinted — hooks, aliases, `core.pager`, `core.fsmonitor`, `core.sshCommand`, `core.editor`, `filter.*`, `diff.*.textconv`, `url.*.insteadOf`, `remote.*.url` | **doctor fails** | `tests/test_runner_and_doctor.py::test_git_tamper_notices_a_planted_hook`, `::test_git_tamper_catches_every_execution_key` |
